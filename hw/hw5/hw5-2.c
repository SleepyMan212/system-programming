...
int main (int argc, char** argv) {
    ...
    while(1) {
        ...
        */
		...
		ent = readdir(dir);
		while(ent!=NULL){
		    if(strcmp(exeName,ent->d_name)==0){
			abPath = p;
			strcat(abPath,"/");
			strcat(abPath,exeName);
			is_find = 1;
	    ...
	    for(int i=0; i<256; i++){
		if(argVect[i]!=NULL) size++;
	    }

	    switch(size){
		case 1:
	   	    if(execle(abPath,exeName,NULL,NULL)==-1){
                	perror("myShell");
                	exit(errno*-1);
	    	    }
		    break;
		case 2:
	   	    if(execle(abPath,exeName,argVect[1],NULL,NULL)==-1){
                	perror("myShell");
                	exit(errno*-1);
	    	    }
		    break;
		case 3:
	   	    if(execle(abPath,exeName,argVect[1],argVect[2],NULL,NULL)==-1){
                	perror("myShell");
                	exit(errno*-1);
	    	    }
		    break;
		case 4:
	   	    if(execle(abPath,exeName,argVect[1],argVect[2],argVect[3],NULL,NULL)==-1){
                	perror("myShell");
                	exit(errno*-1);
	    	    }
		    break;
		case 5:
	   	    if(execle(abPath,exeName,argVect[1],argVect[2],argVect[3],argVect[4],NULL,NULL)==-1){
                	perror("myShell");
                	exit(errno*-1);
	    	    }
		    break;
